...
const findId = function(numId) {
  for(obj in inventory) {
    if(inventory[obj].id === numId) {
      return `Car ${inventory[obj].id} is a ${inventory[obj].car_year} ${inventory[obj].car_make} ${inventory[obj].car_model}`
    }
  }
}
...
console.log(`The last car is ${inventory[obj].car_make} and ${inventory[obj].car_model}`);
...
console.log(inventory.map(car => car.car_model).sort(sortCar));
...
const oldCars = carYears.filter(car => car.car_year);
